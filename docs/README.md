...
The data structure for storing time is defined as follows:

```c
struct Time
{
    unsigned char day; // Day of week
    unsigned short timeOfDay; // Number of PROGRAM_TIME_UNIT_MICROS
                              // microseconds that have elapsed today.
};
```

The data structure requires 3 bytes of storage as it is currently defined.

## `struct Program`

The data structure for storing a program is defined as follows:
```c
struct Program
{
    struct Time start, end;  // Activation and deactivation time
    unsigned short min, max; // Min and max regulated temperature
}
```
_Note that another bit of information must be included: whether the program is active. This information is coded in the sign of_ `start.day` _- a positive sign indicates that the program is active._
...
* `start.day` and `end.day` must store values 0 - 6, which requires 3 bits of storage for each.

* With the current programming resolution of one minute, `start.timeOfDay` and `end.timeOfDay` need only store values from 0 - 1439 (the number of minutes in a day). Therefore, these attributes require only 11 bits of storage each.
